...
public class SeatViewConfig {
    ...
    private void initSeatArray(List<SeatRow> seatDatas) {
        if (seatDatas != null) {
            ...
            for (int rowIndex = 0; rowIndex < seatDatas.size(); rowIndex++) {
                ...
                rowNames[rowIndex] = RowNameUtil.numToAlphabet.get(rowSeatListBean.rowName);
    ...
    private void initThumbSize() {
        ...
        float seatColumnCount = columnCount + SeatViewConfig.SEAT_INLINE_GAP_WIDTH_RATIO * (columnCount - 1) + 2;
        //padding = seatwidth
